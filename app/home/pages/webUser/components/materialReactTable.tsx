...
const MaterialReactTableComponent = <T extends MRT_RowData>({
    ...
    const memoizedColumns = useMemo(
        () =>
            columns.map((column) => ({
                ...
                // 🔹 Preserve custom Cell renderer if it exists
                Cell: column.Cell
                    ? column.Cell
                    : ({ cell }: { cell: MRT_Cell<T> }) => {
                        ...
                        return (
                            <Box
                                sx={{
                                    ...commonStyles,
                                    cursor: enableCellClick ? "pointer" : "default",
                                }}
                                onClick={() =>
                                    enableCellClick &&
        ...
        [columns, enableCellClick, onCellClick, selectedHeaders, toggleHeaderSelection, headerSelection],
    );
    ...
    const table = useMaterialReactTable({
        ...
        muiBottomToolbarProps: {
            sx: {
                backgroundColor: 'background.paper', // lighter and consistent with header
                border: "1px solid",
                borderTop: "none",               // add a top border to separate from table body
                borderColor: "divider",
                borderRadius: "0 0 8px 8px",
                padding: "8px 16px",                  // add some vertical and horizontal padding
                boxShadow: "0 -2px 8px rgba(0,0,0,0.1)",  // subtle shadow on top of footer
                display: "flex",
                alignItems: "center",
                justifyContent: "space-between",     // space out pagination and extra content if any
                labelRowsPerPage: "Rows perasdasd page",
            },
        },
